<template>
  <div>
     <header>
        <nav>
            <svg data-v-c8684834="" width="100%" height="100%" xmlns="http://www.w3.org/2000/svg" version="1.1"><polyline data-v-c8684834="" points="12,18 4,9 12,0" style="fill: none; stroke: rgb(255, 255, 255); stroke-width: 3;"></polyline></svg>
        </nav>
        ...
        <div class="nav">
            ...
            111 (APP专项)
            <span class="act">三个活动
                <svg  data-v-c8684834="" class="footer_arrow"><use data-v-c8684834="" xmlns:xlink="http://www.w3.org/1999/xlink" xlink:href="#arrow-left"></use></svg>
            </span>
        </div>
     </header>
    <nav class="storenav">
    ...
    <div class="storemain">
        <ul class="column">
            <li v-for="(item) in shopDetailList" :key="item.id" @click="navTarget(item.id)" :class="{active:item.id === currentId}" :ref="`col_${item.id}`">{{item.name}}</li>
        </ul>
        <div class="detail" ref="detail" @scroll="location()">
            <dl v-for="list in shopDetailList" :key="list.id" :ref="list.id">
                <dt>{{list.name}} <span>{{list.description}}</span></dt>
                <dd v-for="item in list.foods" :key="item.id">
                    ...
                    <hgroup>
                        ...
                        <span class="sign" v-for="(attr,index) in item.attributes" :key="index" v-show="item.attributes.length > 0 && attr !== null">招聘</span>
                        <p class="des">{{item.description}}</p>
                        <p>{{item.tips}}</p>
                        <span class="flag">{{item.item_id}}</span>
                        <div class="footer">
                            <span><span class="price">￥{{item.satisfy_rate}}</span>起</span>
